...
async function runMigrations(db: Awaited<ReturnType<typeof getDatabase>>) {
  ...
  if (!columns.some((c) => c.name === 'bonus_percent')) {
    await db.execAsync('ALTER TABLE users ADD COLUMN bonus_percent REAL NOT NULL DEFAULT 10');
  }
  ...
  if (!prodCols.some((c) => c.name === 'handling_fee')) {
    await db.execAsync('ALTER TABLE products ADD COLUMN handling_fee REAL NOT NULL DEFAULT 0');
  }

  // Add handling_fee to transaction_items if missing
  const tiCols = await db.getAllAsync<{ name: string }>("PRAGMA table_info(transaction_items)");
  if (!tiCols.some((c) => c.name === 'handling_fee')) {
    await db.execAsync('ALTER TABLE transaction_items ADD COLUMN handling_fee REAL NOT NULL DEFAULT 0');
  }
